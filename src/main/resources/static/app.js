...
class ScholarMindApp {
    constructor() {
        ...
        this.memoryScope = localStorage.getItem('scholarmindMemoryScope') || `browser-${this.generateSessionId()}`;
        localStorage.setItem('scholarmindMemoryScope', this.memoryScope);
    ...
    initializeElements() {
        ...
        this.researchFlowBtn = document.getElementById('researchFlowBtn');
    ...
    bindEvents() {
        ...
        // Research Flow按钮
        if (this.researchFlowBtn) {
            this.researchFlowBtn.addEventListener('click', () => this.triggerResearchFlow());
        }
    ...
    handleFileSelect(event) {
        ...
        if (file) {
            ...
            if (!this.validateFileType(file)) {
                this.showNotification('只支持上传 PDF、TXT 或 Markdown 文件', 'error');
    ...
    // 发送多 Agent 论文研究请求（SSE 流式模式）
    async sendResearchWorkflowRequest(loadingMessageElement) {
        try {
            const response = await fetch(`${this.apiBaseUrl}/research/workflow`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ SessionId: this.sessionId, MemoryScope: this.memoryScope })
            });
            ...
            try {
                while (true) {
                    ...
                    if (done) {
                        // 流结束，更新最终内容
                        if (fullResponse) {
                            console.log('Research Flow 流结束，更新最终内容，长度:', fullResponse.length);
                            this.updateResearchFlowMessage(loadingMessageElement, fullResponse, []);
                        }
                        break;
                    }
                    ...
                    for (const line of lines) {
                        ...
                        console.log('[Research Flow SSE] 收到行:', line);
                        ...
                        if (line.startsWith('id:')) {
                            ...
                            currentEvent = line.substring(6).trim();
                            console.log('[Research Flow SSE] 事件类型:', currentEvent);
                            continue;
                        } else if (line.startsWith('data:')) {
                            const rawData = line.substring(5).trim();
                            console.log('[Research Flow SSE] 数据:', rawData, ', currentEvent:', currentEvent);
                            ...
                            const processJsonMessages = (data) => {
                                ...
                                if (matches && matches.length > 0) {
                                    console.log('[Research Flow SSE] 匹配到', matches.length, '个JSON对象');
                                    for (const jsonStr of matches) {
                                        try {
                                            ...
                                            if (sseMessage.type === 'content') {
                                                ...
                                                console.log('Research Flow 流完成，最终内容长度:', fullResponse.length);
                                                this.updateResearchFlowMessage(loadingMessageElement, fullResponse, []);
                                                return true;
                                            } else if (sseMessage.type === 'error') {
                                                throw new Error(sseMessage.data || '多 Agent 论文研究失败');
                                            }
                                        } catch (e) {
                                            if (e.message.includes('多 Agent')) throw e;
                                            console.log('[Research Flow SSE] 单个JSON解析失败:', jsonStr);
                                        }
                                    }
                                    if (loadingMessageElement) {
                                        this.updateResearchFlowStreamContent(loadingMessageElement, fullResponse);
                                    }
                                    return false;
                                }
                                return null;
    ...
    // 更新多 Agent 论文研究流式内容（实时显示）
    updateResearchFlowStreamContent(messageElement, content) {
        if (!messageElement) return;
        
        // 添加 research-message 类
        messageElement.classList.add('research-message');
    ...
    // 更新多 Agent 论文研究消息（带折叠详情）
    updateResearchFlowMessage(messageElement, response, details) {
        console.log('updateResearchFlowMessage 被调用');
        ...
        if (!messageElement) {
            ...
            return this.addResearchFlowMessage(response, details);
        }
        ...
        if (details && details.length > 0) {
            // 检查是否已存在详情容器
            let detailsContainer = messageElement.querySelector('.research-details');
            if (!detailsContainer) {
                detailsContainer = document.createElement('div');
                detailsContainer.className = 'research-details';
    ...
    // 添加多 Agent 论文研究消息（带折叠详情）- 保留用于兼容性
    addResearchFlowMessage(response, details) {
        const messageDiv = document.createElement('div');
        messageDiv.className = 'message assistant research-message';
    ...
    // 触发多 Agent 论文研究（点击 Research Flow 按钮时直接调用）
    async triggerResearchFlow() {
        if (this.isStreaming) {
        ...
        this.currentResearchFlowMessage = loadingMessage; // 保存消息引用用于后续更新
        ...
        try {
            await this.sendResearchWorkflowRequest(loadingMessage);
        } catch (error) {
            console.error('多 Agent 论文研究失败:', error);
            // 更新消息为错误信息
            if (loadingMessage) {
                const messageContent = loadingMessage.querySelector('.message-content');
                if (messageContent) {
                    messageContent.textContent = '抱歉，多 Agent 论文研究时出现错误：' + error.message;
                }
            }
        } finally {
            this.isStreaming = false;
            this.currentResearchFlowMessage = null;
            this.updateUI();
        }
    }
    ...
    showLoadingOverlay(show) {
        if (this.loadingOverlay) {
            if (show) {
                ...
                if (loadingText) loadingText.textContent = '多 Agent 论文研究中，请稍候...';
                if (loadingSubtext) loadingSubtext.textContent = '正在检索论文证据并规划实验复现';
                // 防止页面滚动
