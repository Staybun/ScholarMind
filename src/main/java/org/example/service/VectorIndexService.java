...
public class VectorIndexService {
    ...
    public IndexingResult indexDirectory(String directoryPath) {
        ...
        try {
            ...
            // 获取所有支持的文件
            File[] files = directory.listFiles((dir, name) -> isSupportedPaperFile(name));
    ...
    public void indexSingleFile(String filePath) throws Exception {
        ...
        // 1. 解析论文并清洗文本
        PaperDocument paperDocument = paperDocumentParserService.parse(path);
        if (paperDocument.getContent() == null || paperDocument.getContent().isBlank()) {
            throw new IllegalArgumentException("论文未解析出有效文本，可能是扫描版 PDF 或文件内容为空: " + filePath);
        }
        logger.info("读取论文: {}, 标题: {}, 页数: {}, 清洗后长度: {} 字符",
                path, paperDocument.getTitle(), paperDocument.getPageCount(), paperDocument.getContent().length());

        // 2. 删除该文件的旧数据（如果存在）
        deleteExistingData(path.toString());

        // 3. 文档分片
        List<DocumentChunk> chunks = chunkService.chunkPaper(paperDocument);
        logger.info("论文分片完成: {} -> {} 个分片", filePath, chunks.size());
    ...
    private boolean isSupportedPaperFile(String name) {
